  ...
  if (baseUrl && platformToken) {
    try {
      ...
      while (hasMore) {
        ...
        if (offset === 0) {
          firstPageDebug = JSON.stringify({ topKeys: Object.keys(json), dataKeys: Object.keys(data), preview: rawText.slice(0, 600) });
          debugLog("get_contratos:raw", JSON.parse(firstPageDebug));
        }
